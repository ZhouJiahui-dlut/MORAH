...
class GraphCAGE(nn.Module):
    ...
    def forward(self, text_vertex, audio_vertex, video_vertex, adj_t, adj_a, adj_v, batch_size):
        ...
        # return logits
        nodes_t = torch.cat([logits_t_fir, logits_t_sec], -1)
        nodes_a = torch.cat([logits_a_fir, logits_a_sec], -1)
        nodes_v = torch.cat([logits_v_fir, logits_v_sec], -1)
        return nodes_t, nodes_a, nodes_v
